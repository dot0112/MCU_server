...
// Wi-Fi 설정
String ssid = "LAB821(AI&ASIC)";
String password = "deucomputer821";

// 서버 설정
IPAddress serverIP(192,168,50,126);
//String serverIP = "192.168.50.126";
int serverPort = 7000;
...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    ...
    if (wifiConnectCount >= 10) {
      Serial.println("\nNOWIFI");
      __time = millis();
      while (millis() - __time < 10000) {
        if (Serial.available()) {
          String data = Serial.readString();
          int index = data.indexOf(':');
          ssid = data.substring(0, index);
          password = data.substring(index + 1, data.length() - 2);
          Serial.println(ssid);
  ...
  while (true) {
    if (!client.connect(serverIP, serverPort)) {
      Serial.println("NOSERVER");
      __time = millis();
      while (millis() - __time < 10000) {
        if (Serial.available()) {
          String data = Serial.readString();
          int index = data.indexOf(':');
          serverIP.fromString(data.substring(0, index));
          serverPort = data.substring(index + 1).toInt();
          break;
        }
      }
    } else {
